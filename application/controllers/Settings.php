...
class Settings extends CI_Controller {
    ...
    public function profile_admin_utama()
	{
        if ($this->session->userdata('logged_in') == true AND $this->session->userdata('id_user_level') == 3) {
        
        $data['user'] = $this->m_user->get_user_by_id($this->session->userdata('id'))->result_array();
        $this->load->view('admin_utama/profile_edit', $data);
    ...
    public function edit_data_admin_utama($id){
        if ($this->session->userdata('logged_in') == true AND $this->session->userdata('id_user_level') == 3) {
			...
			redirect('Settings/profile_admin_utama');
